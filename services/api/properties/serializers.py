...
class PropertySerializer(serializers.ModelSerializer):
    ...
    def _clean_publication_status(self, validated_data, *, creating=False):
        """Evita que usuarios no staff manipulen publication_status directamente."""
        request = self.context.get('request')
        user = getattr(request, 'user', None)
        ...
        if not user or not user.is_authenticated or not user.is_staff:
            ...
            if creating:
                default_status = Property._meta.get_field('publication_status').get_default()
                validated_data.setdefault('publication_status', default_status)
